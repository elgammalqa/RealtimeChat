...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	//---------------------------------------------------------------------------------------------------------------------------------------------
	if ([text length] != 0)
		predicate = [NSPredicate predicateWithFormat:@"isArchived == NO AND isDeleted == NO AND details CONTAINS[c] %@", text];
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	ChatGroupView *chatGroupView = [[ChatGroupView alloc] initWith:groupId];
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	UIAlertController *alert = [UIAlertController alertControllerWithTitle:nil message:nil preferredStyle:UIAlertControllerStyleActionSheet];
	//---------------------------------------------------------------------------------------------------------------------------------------------
	[alert addAction:[UIAlertAction actionWithTitle:@"10 hours" style:UIAlertActionStyleDefault
											handler:^(UIAlertAction *action) { [self actionMute:index until:10]; }]];
	[alert addAction:[UIAlertAction actionWithTitle:@"7 days" style:UIAlertActionStyleDefault
											handler:^(UIAlertAction *action) { [self actionMute:index until:168]; }]];
	[alert addAction:[UIAlertAction actionWithTitle:@"1 month" style:UIAlertActionStyleDefault
											handler:^(UIAlertAction *action) { [self actionMute:index until:720]; }]];
	[alert addAction:[UIAlertAction actionWithTitle:@"1 year" style:UIAlertActionStyleDefault
											handler:^(UIAlertAction *action) { [self actionMute:index until:8760]; }]];
	[alert addAction:[UIAlertAction actionWithTitle:@"Cancel" style:UIAlertActionStyleCancel handler:nil]];
	//---------------------------------------------------------------------------------------------------------------------------------------------
	[self presentViewController:alert animated:YES completion:nil];
}
...
{
	DBChat *dbchat = dbchats[index];
	NSString *chatId = dbchat.chatId;
	//---------------------------------------------------------------------------------------------------------------------------------------------
	NSDate *dateUntil = [[NSDate date] dateByAddingTimeInterval:(hours * 60 * 60)];
	long long mutedUntil = [dateUntil timestamp];
	//---------------------------------------------------------------------------------------------------------------------------------------------
	[Status updateMutedUntil:chatId mutedUntil:mutedUntil];
}
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	NSString *chatId = dbchat.chatId;
	//---------------------------------------------------------------------------------------------------------------------------------------------
	[Status updateMutedUntil:chatId mutedUntil:0];
}
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	[cell loadImage:dbchats[indexPath.row] tableView:tableView];
	//---------------------------------------------------------------------------------------------------------------------------------------------
	return cell;
}
